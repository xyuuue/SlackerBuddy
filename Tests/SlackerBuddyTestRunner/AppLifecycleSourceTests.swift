...
let appLifecycleSourceTests: [TestCase] = [
    ...
    TestCase(name: "app is branded as SlackerBuddy with custom paw icons") {
        ...
        try expect(buildScript.contains("APP_VERSION=\"2.0.1\""), "Packaged app should declare the current 2.0.1 version")
        try expect(buildScript.contains("CFBundleShortVersionString"), "Packaged app should expose a user-facing version")
        try expect(buildScript.contains("CFBundleVersion"), "Packaged app should expose a bundle build version")
    ...
    TestCase(name: "app uses provided SlackerBuddy icon assets") {
        ...
        try expect(fnv1a64Digest(of: repoMenuIcon) == 0x779a56e082039e3d, "Expected menu bar icon to use the current paw artwork")
        try expect(appIconSize.width == 1024 && appIconSize.height == 1024, "Expected app icon to be a standard 1024px square asset")
    ...
    TestCase(name: "app icon is full bleed without a white canvas border") {
        ...
        try expect(
            ...
            "App icon should not include the white source canvas on its edges or corners"
        )
    },
    ...
    TestCase(name: "release website introduces SlackerBuddy and links the DMG") {
        ...
        try expect(siteSource.contains("<link rel=\"icon\" type=\"image/png\" href=\"assets/slackerbuddy-app-icon.png?v=2.0.1\">"), "Release page should use the SlackerBuddy app icon as its browser favicon")
        try expect(siteSource.contains("<link rel=\"apple-touch-icon\" href=\"assets/slackerbuddy-app-icon.png?v=2.0.1\">"), "Release page should use the SlackerBuddy app icon for saved site icons")
        try expect(siteSource.contains("Latest macOS version: 2.0.1"), "Release page should mark 2.0.1 as the latest macOS version")
        try expect(siteSource.contains("最新 macOS 版本：2.0.1"), "Chinese release copy should mark 2.0.1 as the latest macOS version")
...
private func appIconHasWhiteCanvasBorder(at path: String) throws -> Bool {
    ...
    return samplePoints.contains { point in
        ...
        return color.alpha >= 250 && color.red >= 245 && color.green >= 245 && color.blue >= 245
    }
}
